...
class PbmImage private (var imageBuffer : Array[Array[Boolean]]) {
  ...
  private def checkDimensionWithinBoundaries(x : Int, y : Int) : Unit = {
    assert(x < xDim && y < yDim && x > 0 && y > 0, s"the pixel ($x,$y) is not within the image boundaries ($xDim,$yDim)")
  }
  ...
  private def writePlainPbm(path : String) : Unit = {
    ...
    try {
      ...
      imageBuffer.flatten.grouped(70).foreach {
        segment =>
          outStream.write(segment.map(boolToString).mkString("\n", "", "\n").getBytes(encoding))
      }
    } finally {
      outStream.close()
    }
